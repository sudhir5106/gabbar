...
/*	print_r($_POST);
	...
	try
	{
		...
		$tblvalues=array($_POST['title'],$gallary_image,$_POST['description'],$_POST['htitle'],$_POST['hdescription'],1,1);
		$res=$db->valInsert("ongoing_projects",$tblfield,$tblvalues);
		if(!$res)
		{
...
if($_POST['type']=="editProject")
{
	...
	try
	{
		...
		if($_REQUEST['imageupload']=='')
		{
			...
			$select=$db->ExecuteQuery("SELECT Project_Image FROM ongoing_projects WHERE Project_Id='".$_REQUEST['id']."'");
			if(count($select)>0)
				{
	    ...
	    $res=$db->updateValue('ongoing_projects',$tblfield,$tblvalues,$condition);
		if(!$res)
		{
			throw new Exception('c');
		}
...
if($_POST['type']=="delete")
{
	...
	try
	{
		...
		$res=$db->ExecuteQuery("SELECT Project_Image FROM ongoing_projects WHERE Project_Id='".$_REQUEST['id']."'");
