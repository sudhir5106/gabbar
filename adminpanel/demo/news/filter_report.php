...
if($_REQUEST['date']!='')
{
	$condition.=' AND Date="'.date('Y-m-d',strtotime($_REQUEST['date'])).'"';
}
//Check Here IF Langauge is Empty For Filter
if($_REQUEST['heading']!='')
{
	$condition.=' AND Heading LIKE "%'.$_REQUEST['heading'].'%"';
}

//Get Here News  List
$sql="SELECT Id, CASE WHEN Date=0 THEN '----' ELSE DATE_FORMAT(Date,'%d-%m-%Y') END  PDate, CASE WHEN Status=1 THEN 'Active' ELSE 'Not Active' END Status,CASE WHEN Status=0 THEN 'Show' ELSE 'Hide' END PStatus, Description, Heading  FROM news WHERE 1=1 ".$condition." ORDER BY Id DESC";
...
<form name="planresult" id="planresult">
        ...
        <input type="hidden" name="heading" id="heading" value="<?php echo @$_REQUEST['heading']; ?>" />
