...
//Check Here IF Faculty Name  is Empty For Filter
if($_REQUEST['faculty_name']!='')
{
	$condition.=' AND FC.First_Name = "'.trim($_REQUEST['faculty_name']).'"';
} 
//Check Here Mobile No is Empty For Filter
if($_REQUEST['mobile']!='')
{
	$condition.=' AND FC.Mobile="'.$_REQUEST['mobile'].'"';
}
//Check Here Email id is Empty For Filter
if($_REQUEST['email']!='')
{
	$condition.=' AND FC.email="'.$_REQUEST['email'].'"';
}


//Get Here Faculities List
 $sql="SELECT Faculty_Id, CONCAT(First_Name,' ', Middle_Name,' ',Last_Name) AS Faculty_Name, DATE_FORMAT(DOB,'%d-%m-%Y') AS DOB, Father_Name, Qualification, Address,Mobile, Email, DATE_FORMAT(Join_Date,'%d-%m-%Y') AS Join_Date, Profile_Image, CT.name AS Country_Name, SS.name AS State_Name, CS.name AS City_Name FROM faculties FC LEFT JOIN countries CT ON CT.id=FC.Country LEFT JOIN states SS ON SS.id=FC.State LEFT JOIN cities CS ON CS.id=FC.City WHERE 1=1 ".$condition." ORDER BY Faculty_Id DESC";
		...
		while($val=mysql_fetch_array($rs)){ ?>
            <tr>
             ...
             <td><?php echo $val['Qualification'];?></td>
              ...
              <td><?php if($val['Profile_Image']!=''){ echo '<img src="'.PATH_UPLOAD_IMAGE.'/profile/thumb/'.$val['Profile_Image'].'"/>'; } ?></td>
              <td><a href="#" class="view-details" value="<?php echo $val['Faculty_Id']?>">View</a> | <a href="edit.php?id=<?php echo $val['Faculty_Id'];?>">Edit</a> | <a class="delete" href="#" id="<?php echo $val['Faculty_Id'];?>"> Delete</a>
              
              <div id="show<?php echo $val['Faculty_Id'];?>" title="Faculty Details" style="display:none; width:500px !important; ">
                  <p> <strong>Faculty Name</strong> - <?php echo $val['Faculty_Name'];?><br/> 
                      ...
                      <strong>Profile Image </strong> - <?php if($val['Profile_Image']!=''){ echo '<img src="'.PATH_UPLOAD_IMAGE.'/profile/'.$val['Profile_Image'].'"/>';} ?><br/>
                  </p>
                </div>
